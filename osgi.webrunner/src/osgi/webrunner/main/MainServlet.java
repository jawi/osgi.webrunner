...
public class MainServlet extends HttpServlet
{
  ...
  protected void doGet( HttpServletRequest req, HttpServletResponse resp ) throws ServletException, IOException
  {
    ...
    if ( delegateCompName != null && delegateCompName.matches( "\\w+" ) )
    {
      ...
      {
        writer.printf( "<p>Component '%s' not found!</p>", delegateCompName );
      }
    }
    else
    {
      ...
      writer
          .print( "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"utf-8\"/><title>OSGi web runner</title><style>body { font-family: sans-serif; font-size: 10pt; } .iframe { border: solid 1px #ddd; margin: 0.7em; padding: 0.3em; width: 40em; height: 10em; float: left; }</style></head><body>" );
      writer.print( "<h1>OSGi web runner</h1>" );
      writer.print( "<p>This small example shows how to dynamically add/remove servlets.</p>" );
      writer.printf( "<p>Found <strong>%d</strong> components!</p>", componentNames.size() );

      for ( String componentName : componentNames )
      {
        writer.printf( "<div class='iframe' id='if%s' data-src='?c=%s'></div>", componentName, componentName );
      }

      writer
          .println( "<script src='https://ajax.googleapis.com/ajax/libs/jquery/1.5.2/jquery.min.js'></script>"
  ...
  private Servlet getWebComponent( String name )
  {
    ...
    try
    {
      String filter = String.format( "(&(%s=%s)(name=%s))", Constants.OBJECTCLASS, Servlet.class.getName(), name );
  ...
  private Collection<String> getWebComponentNames()
  {
    ...
    try
    {
      ...
      String filter = String.format( "(&(%s=%s)(name=*))", Constants.OBJECTCLASS, Servlet.class.getName() );
  ...
  /**
   * Simple comparator implementation that sorts on the service ID of two given
   * service references.
   */
  static class ServiceReferenceComparator implements Comparator<ServiceReference>
  {
    @Override
    public int compare( ServiceReference o1, ServiceReference o2 )
    {
      Long id1 = ( Long )o1.getBundle().getBundleId();
      Long id2 = ( Long )o2.getBundle().getBundleId();
